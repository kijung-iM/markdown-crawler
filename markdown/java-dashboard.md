...
![에이전트 연결 상태](/img/apm-dashboard-check-agent.png)
...
![에이전트 선택하기](/img/apm-dashboard-select-agent.png)
...
### 분류별 에이전트 모니터링[​](#분류별-에이전트-모니터링 "분류별 에이전트 모니터링에 대한 직접 링크")

![분류별 에이전트 선택하기](/img/apm-dashboard-select-agent-by-category.png)
...
### 에이전트 관련 위젯[​](#에이전트-관련-위젯 "에이전트 관련 위젯에 대한 직접 링크")

애플리케이션에 설치한 에이전트를 통해서 애플리케이션의 정보를 확인할 수 있습니다.

#### 애플리케이션 요약

![애플리케이션 요약](/img/agent-dashboard-summary.png)

프로젝트에 등록한 애플리케이션의 실행 상태 및 개수, CPU 코어 수, Hosts 수를 표시합니다.

### 트랜잭션 관련 위젯[​](#트랜잭션-관련-위젯 "트랜잭션 관련 위젯에 대한 직접 링크")
...
#### 액티브 트랜잭션 스피드[​](#-1 "-1에 대한 직접 링크")

![액티브 트랜잭션 스피드](/assets/images/speedmeter-ca3c8c20cd8eb551279a5633c2df3448.png)

현재 진행 중인 트랜잭션(가운데 영역)과 종료된 트랜잭션(오른쪽 영역) 현황을 확인할 수 있습니다. 왼쪽에서 오른쪽으로 지나가는 물방울 모양의 아이콘은 트랜잭션을 의미합니다. 액티브 트랜잭션의 상태를 속도(Normal, Slow, Very Slow)로 색상으로 분류하여 우측 상단에 개수로 표시합니다.

#### 액티브 트랜잭션[​](#active-trs "active-trs에 대한 직접 링크")

![액티브 트랜잭션](/img/apm-dashboard-active-tx.png)
...
![트랜잭션 목록](/img/apm-dashboard-active-tx-popup.png)

노트
	+ 트랜잭션 목록에서 선택한 항목이 이미 종료된 트랜잭션일 경우 트랜잭션 보기 버튼을 선택하세요. 트랜잭션 정보 창이 나타납니다. 트랜잭션 정보 창에 대한 자세한 내용은 [다음 문서](/java/trs-profile)를 참조하세요.
	+ 트랜잭션에 대한 자세한 내용은 [다음 문서](/java/track-transactions-intro)를 참조하세요.

#### 에이전트 액티브 트랜잭션[​](#active-trs-agent "active-trs-agent에 대한 직접 링크")

![에이전트 액티브 트랜잭션](/img/apm-dashboard-active-tx-agent.png)
...
#### 액티브 스테이터스[​](#active-status "active-status에 대한 직접 링크")

![액티브 스테이터스](/img/apm-dashboard-active-status.png)
...
노트DB 연결 지연과 관련한 자세한 내용은 [다음 문서](/best-practice-guides/about-apm-dbc)를 참조하세요.
...
### 서비스 관련 위젯[​](#서비스-관련-위젯 "서비스 관련 위젯에 대한 직접 링크")

서비스 관련 위젯을 통해 애플리케이션 성능 튜닝을 위한 지표로 활용할 수 있습니다.

#### Apdex[​](#-3 "-3에 대한 직접 링크")

![Apdex](/assets/images/apm-dashboard-apdex-4beb13d03ae3217f150ef39562cb6ed1.png)
...
노트만족 S 기본값은 관리 > 에이전트 설정 메뉴에서 변경할 수 있습니다. 설정 키 값은 `apdex_time`입니다. 에이전 트 버전 2.0 이상만 지원합니다.

#### TPS[​](#-4 "-4에 대한 직접 링크")

![TPS](/assets/images/apm-dashboard-tps-29541030b3a5bbb465a7ab439dd26330.png)
...
노트TPS, 응답시간, CPU 사용률 분석에 대한 자세한 내용은 [다음 문서](/best-practice-guides/about-apm-dashboard#analysis-user-service-resource)를 참조하세요.

#### 평균 응답시간[​](#-5 "-5에 대한 직접 링크")

![평균 응답시간](/img/apm-dashboard-avg-response-time.png)
...
### 리소스 관련 위젯[​](#리소스-관련-위젯 "리소스 관련 위젯에 대한 직접 링크")

#### 시스템 CPU[​](#-6 "-6에 대한 직접 링크")

![시스템 CPU](/img/apm-dashboard-cpu.png)

애플리케이션 서버의 CPU 사용량입니다. 실시간으로 CPU 사용량 변화 추이를 파악할 수 있습니다.

노트TPS, 응답시간, CPU 사용률 분석에 대한 자세한 내용은 [다음 문서](/best-practice-guides/about-apm-dashboard#analysis-user-service-resource)를 참조하세요.

#### 힙 메모리[​](#heap-memory "heap-memory에 대한 직접 링크")

![힙 메모리](/img/apm-dashboard-heap-memory.png)
...
팁힙 메모리 차트 분석에 대한 자세한 내용은 다음 링크를 참조하세요.
...
### 사용자 관련 위젯[​](#사용자-관련-위젯 "사용자 관련 위젯에 대한 직접 링크")

#### 동시접속 사용자[​](#-7 "-7에 대한 직접 링크")

![동시접속 사용자](/img/apm-dashboard-realtime-user.png)

실시간 브라우저 사용자 수를 보여줍니다. 5초마다 최근 5분 이내에 트랜잭션을 일으킨 사용자를 중복없이 카운팅해 표시합니다. 사용자 브라우저의 IP를 기반으로 카운팅합니다.

노트* 에이전트 설정에서 사용자 구분을 위해 IP를 사용하거나 쿠키를 사용할 수 있습니다. 자세한 내용은 [다음 문서](/java/agent-additional-option)를 참조하세요.
